...
const (
	GetOtherFormsQuery = `SELECT f.id, f.title, f.description, COALESCE(l.count, 0) AS count, EXISTS(SELECT * FROM likes_forms WHERE user_id = $1 AND form_id = f.id) AS is_liked, COUNT(SELECT id FROM comments c WHERE c.form_id = f.id) AS count_votes, f.created_at, f.expires_at FROM forms f LEFT JOIN likes l ON l.form_id = f.id WHERE creator_id != $1 AND f.expires_at > NOW() AND f.private_key = false LIMIT 10`
