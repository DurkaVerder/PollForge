...
func GetUserFormsRequest(userId int) (*sql.Rows, error) {
	rows, err := Db.Query("SELECT f.id, f.creator_id, t.name, f.title, f.description, f.link, f.private_key, f.expires_at, f.created_at FROM forms f LEFT JOIN themes t ON f.themes_id = t.id WHERE creator_id = $1", userId)
	if err != nil {
